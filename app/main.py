...
# Ensure the script can find the ai module in parent directory
import sys
    ...
    firebase_admin.initialize_app(
        cred,
        {
            "storageBucket": os.environ.get(
                "FIREBASE_STORAGE_BUCKET", "some-bucket.appspot.com"
            )
        },
    )
...
def load_whisper_model():
    return whisper.load_model(
        "base"
    )  # can use "small", "medium", "large" for accuracy/speed
...
#    PAGE: Voice Logging
    ...
    st.info(
        "Record your expense by clicking the microphone (e.g., 'I spent 150 rupees on lunch')"
    )
    audio_bytes = st.audio_input("Record your expense")
    if audio_bytes:
        ...
        amount_voice = st.number_input(
            "Amount (from voice or enter manually)",
            min_value=1.0,
            step=1.0,
            value=extracted_amt or 1.0,
        )
...
#    PAGE: Insights (Charts, Stats)
    ...
    docs = (
        db.collection("users").document(get_user_id()).collection("expenses").stream()
    )
    import pandas as pd
    ...
    if not df.empty:
        ...
        if not cat_sum.empty and cat_sum.iloc[0] > 0:
            st.warning(
                f"Highest spending: **{cat_sum.index[0]}** (₹{cat_sum.iloc[0]:,.2f})"
            )
    else:
...
#    PAGE: Settings
    ...
    user_id = st.text_input(
        "Set your User ID (for privacy & syncing)", value=get_user_id()
    )
